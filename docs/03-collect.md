...
| `fee_bearer` | string | ❌ | `MERCHANT` (default) / `CUSTOMER` — see [Fees](07-fees-and-fee-bearer.md) |
| `idempotency_key` | string | ❌ | alternative to the header (see [Idempotency](06-idempotency.md)) |

**Currency and country must travel together and be consistent** (e.g. `CM` with
`XAF`). See the [Reference](10-reference.md) for the currency/country map and the
...
2. Optionally receive a **webhook** push — but treat webhooks as a hint and
   **still confirm via status** before acting. See [Webhooks](08-webhooks.md).
